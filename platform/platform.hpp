...
public:
  ...
  std::string const & WritableDir() const { ASSERT(!m_writableDir.empty(), ("Writable dir is empty")); return m_writableDir; }
  ...
  std::string const & ResourcesDir() const { ASSERT(!m_resourcesDir.empty(), ("Resources dir is empty")); return m_resourcesDir; }
